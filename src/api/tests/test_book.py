...
test_data_path = (
    Path(__file__).parent.parent.parent / "api/data/book_failed_paramter.json"
)
...
class TestBook:
    ...
    def test_book_case_03(self, book_client):
        response = book_client.create_book(
            ...
            files={
                "cover": open(
                    Path(__file__).parent.parent.parent / "assets/album-cover.jpeg",
                    "rb",
                )
            },
        )
    ...
    def test_book_case_04(self, book_create, book_client):
        response = book_client.get_book_list({"pageSize": 10, "pageNum": 1, "name": ""})
    ...
    def test_book_case_06(self, db_cursor, book_create, book_client):
        ...
        # 调用阅读记录接口
        response = book_client.book_read_record(
            book_id, headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 200
