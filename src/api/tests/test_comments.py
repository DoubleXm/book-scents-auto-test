...
test_data_path = Path(__file__).parent.parent.parent / 'api/data/comment_failed_paramter.json'
...
class TestComments:
    ...
    def test_comments_case_04(self, book_create,comment_client, test_input, test_expect):
        token = book_create['token']
        book_id = book_create['response'].json()['id']
        
        response = comment_client.create_comment(book_id, test_input, headers={ 'Authorization': f'Bearer {token}' })
