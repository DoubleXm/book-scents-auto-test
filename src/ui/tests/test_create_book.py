...
test_create_book_data_path = Path(__file__).parent.parent.parent / 'ui/data/create_book_paramter.json'
...
class TestCreateBook:
    ...
    @pytest.mark.parametrize("test_input, test_expect", formatter_parameterize_data(test_create_book_data_path))
    ...
    def test_create_book_04(self, create_book_page, test_input, test_expect):
        ...
        if "error_message" in test_expect:
            ...
            expect(create_book_page.error_message).to_have_text(test_expect["error_message"])
        if "warning_message" in test_expect:
            expect(create_book_page.warning_message).to_be_visible()
            expect(create_book_page.warning_message).to_have_text(test_expect["warning_message"])
